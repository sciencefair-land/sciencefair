...
function Datasource (key) {
  ...
  self.ensureMetaFile = through.obj((entry, enc, next) => {
    if (/^meta.*json$/.test(entry.name)) {
      console.log('found metadata', entry.name)
      if (self.archive.isEntryDownloaded(entry)) {
  ...
  self.syncMetadata = cb => {
    ...
    self.updateStats({ totalBlocks: end })
    const pageSize = 1000
    ...
    const pageMetadata = () => {
      ...
      const done = err => {
        ...
        console.log('metadata synced', self.stats)
  ...
  self.download = (article, cb) => {
    ...
    const hit = through.obj((data, enc, cb) => {
      const re = new RegExp(`^articles\/${article.path}`)
      if (re.test(data.name)) hits.push(data)
      cb()
    })

    pump(self.archive.list({ live: false }), hit, err => {
      ...
      const stats = {
        ...
        files: hits.length,
        progress: 0,
        totalBlocks: sum(hits.map(h => h.blocks)),
        blocksDownloaded: sum(hits.map(self.archive.countDownloadedBlocks))
      }
      self._downloads.push(stats)

      const updateStats = () => {
        console.log('harro darling')
        const blocks = sum(hits.map(self.archive.countDownloadedBlocks))
        console.log('bits', blocks, stats.totalBlocks)
        stats.blocksDownloaded = blocks
        stats.progress = (blocks / stats.totalBlocks) * 100
      }

      updateStats()

      const timerId = setInterval(updateStats, 200)
      const alldone = require('./alldone')(hits.length, err => {
        if (err) cb(err)
      ...
      hits.forEach(hit => self.archive.download(hit, alldone))
    })
  }
