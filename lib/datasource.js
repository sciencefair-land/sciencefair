...
function Datasource (key) {
  ...
  self.download = article => {
    ...
    const hit = through.obj((data, enc, cb) => {
      const re = new RegExp(`^${article.path}`)
      if (re.test(data.name)) hits.push(data)
      cb()
    })

    pump(self.archive.list, hit, err => {
      ...
      const totalBlocks = sum(hits.map(h => h.blocks))
      const updateStats = () => {
        const blocks = sum(hits.map(
          h => self.archive.countDownloadedBlocks(hit)
        ))
        stats.progress = blocks / totalBlocks * 100
      }

      const timerId = setInterval(updateStats, 200)
      const alldone = require('./alldone')(hits.length, () => {
  ...
  self.downloads = () => self._downloads.map(d => {
    d.source = self.key
    return d
  })
}
