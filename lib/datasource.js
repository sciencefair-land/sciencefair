...
function Datasource (key) {
  ...
  self.updateStats = update => {
    merge(self.stats, update)
    const newstats = {}
    merge(newstats, self.data())
    delete newstats.stats.peers // don't save peers
    self.statsdb.put('stats', newstats, err => {
      if (err) {
  ...
  self.connect = cb => {
    ...
    swarm.on('connection', (peer, type) => {
      self.updateStats({ peers: swarm.connections.length })
      peer.on('close', () => {
        self.updateStats({ peers: swarm.connections.length })
      })
    })
    ...
    const countArticles = () => {
      let articleCount = self.stats.articleCount || 0
      self.archive.list({ live: false }).on('data', entry => {
        ...
        'close', () => self.updateStats({ articleCount: articleCount })
      ).on(
        'end', () => self.updateStats({ articleCount: articleCount })
      )
    }
    ...
    try {
      loadjson()
      if (!self.stats.articleCount) countArticles()
    } catch (e) {
      self.archive.list({ live: false }).on('data', entry => {
        if (entry.name === '_feed.json') {
          fetchjson(entry)
    ...
    if (self.active && !(self.metadataSync.done === self.metadataSync.total)) {
  ...
  self.syncMetadata = cb => {
    ...
    var end
    var metadataPos = self.stats.metadataSync.done || 0
    const pageSize = 100
  ...
  // close all databases
  self.close = cb => self.archive.close(
    () => self.statsdb.close(
      () => self.db.close(() => fs.remove(self.datadir, cb))
    )
  )
